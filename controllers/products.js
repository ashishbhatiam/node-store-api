...
const getAllProductsStatic = async(req, res) => {
    const products = await Product.find({price: {$gt: 59}}).sort('price').select('name price')
    res.status(200).json({ products, total: products.length})
}

const getAllProducts = async(req, res) => {
    const { featured, name, price, company, sort, fields, numericFilters } = req.query;      /* Destructuring Method */
    const queryObj = {}
    if(featured){
